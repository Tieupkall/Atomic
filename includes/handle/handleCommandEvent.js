    ...
    return function ({ event }) {
        ...
        for (const eventReg of eventRegistered) {
            ...
            try {
                ...
                // Kiểm tra cmd và handleEvent function
                if (cmd && typeof cmd.handleEvent === "function") {
                    cmd.handleEvent(Obj);
                }
            } catch (error) {
                console.error(`Error in handleEvent for ${cmd?.config?.name || 'unknown'}:`, error);
                logger(global.getText('handleCommandEvent', 'moduleError', cmd?.config?.name || 'unknown'), 'error');
            }
        }
    };
};