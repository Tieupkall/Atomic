...
function formatThreadGraphQLResponse(data) {
  try {
    ...
    // Check if messageThread exists and has required properties
    if (!messageThread) {
      console.error("GetThreadInfoGraphQL", "messageThread is null or undefined!");
      return { err: new Error("messageThread is null or undefined") };
    }
    
    if (!messageThread.thread_key) {
      console.error("GetThreadInfoGraphQL", "thread_key is missing from messageThread!");
      return { err: new Error("thread_key is missing from messageThread") };
    }
  ...
  return {
    ...
    participantIDs: messageThread.all_participants ? messageThread.all_participants.edges.map(d => d.node.messaging_actor.id) : [],
    userInfo: messageThread.all_participants ? messageThread.all_participants.edges.map(d => ({
    ...
    // @Undocumented
    reactionsMuteMode: messageThread.reactions_mute_mode ? messageThread.reactions_mute_mode.toLowerCase() : 'none',
    mentionsMuteMode: messageThread.mentions_mute_mode ? messageThread.mentions_mute_mode.toLowerCase() : 'none',
    isPinProtected: messageThread.is_pin_protected || false,
...
module.exports = function(defaultFuncs, api, ctx) {
  return function getThreadInfoGraphQL(threadID, callback) {
    ...
    //* những code tương tự multi thread như này đều có thể là copy idea 🧐
    ...
    defaultFuncs
      ...
      if (resData.error) {
        callback(resData.error, null);
        throw resData;
      }
      
      if (!resData || resData.length === 0) {
        var error = new Error("No data received from Facebook API");
        callback(error, null);
        throw error;
      }
      ...
      resData.map(function (x, y) {
        var threadData = formatThreadGraphQLResponse(x["o"+y].data);
        if (threadData.err) {
          log.error("getThreadInfoGraphQL", "Error formatting thread data:", threadData.err.message);
      ...
      if (ThreadInfo.length === 0) {
        var error = new Error("No valid thread data could be processed");
        callback(error, null);
        return;
      }
