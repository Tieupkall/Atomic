    ...
    if (!DefaultPassWord) return logger.Warning("DefaultPassWord Is Requirements",function() { process.exit(0); });
      try {
        if (!Database().has('Security')) { 
          ...
          Database().set('Security',JSiON.stringify(Obj));
            ...
            var AppState_Decrypt;
              if (!Already_Action.Decode.Status) {
                try {
                  ...
                  if (!AppState_Decrypt || AppState_Decrypt.length === 0) {
                  ...
                  console.log("AppState decryption error:", error.message);
                  throw new Error("AppState is corrupted or password is incorrect. Please replace your AppState.");
                }
              }
              else {
