...
module.exports.run = async function({ api, event, client, __GLOBAL }) {
    // Tạo cooldown để tránh đổi tên liên tục
    if (!global.autoSetNameCooldown) {
        global.autoSetNameCooldown = new Map();
    }

    const now = Date.now();
    const threadID = event.threadID;
    const lastRun = global.autoSetNameCooldown.get(threadID) || 0;

    // Chỉ cho phép đổi tên mỗi 5 phút
    if (now - lastRun < 300000) {
        return;
    }

    global.autoSetNameCooldown.set(threadID, now);
